            ...
            for t in set(IDENT.findall(body)):
                ...
                if r is None:
                    ...
                    if not r and "." in t:
                        parts_t = t.split(".")
                        alts = []
                        for k in range(len(parts_t) - 1, 0, -1):      # drop TRAILING comps
                            alts.append(".".join(parts_t[:k]))
                        for k in range(1, len(parts_t)):              # drop LEADING comps
                            alts.append(".".join(parts_t[k:]))
                        for a in alts:
                            c2 = {a} if a in byfull else suffix.get(a, frozenset())
